...
public:
  ...
  void SetResRelativePose(
    ...
    H_.block<15, 15>(0, 0) += J_m.transpose() * J_m;
    // b. H_mr:
    H_.block<15, 15>(0, 15) += J_m.transpose() * J_r;
    // c. H_rm:
    H_.block<15, 15>(15, 0) += J_r.transpose() * J_m;
    // d. H_rr:
    H_.block<15, 15>(15, 15) += J_r.transpose() * J_r;
    ...
    b_.block<15, 1>(0, 0) += J_m.transpose() * residuals;
    // a. b_r:
    b_.block<15, 1>(15, 0) += J_r.transpose() * residuals;
  }
